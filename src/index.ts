...
// pretend to ai it.
    // ...
    const moderationOut = await ai.moderations.create({
        input: content,
    }).then(d => d.results[0])
    if ([...new Set(Object.values(moderationOut.categories))].includes(true)) {
        ...
        await par.client.chat.update({
            ...
            thread_ts: par.event.ts,
            channel: par.event.channel,
            text: `:x: Harmful content detected.\n>Follwing subjects:  ${Object.entries(moderationOut.categories).filter(e => e[1]).map(e => e[0]).join(', ')}`,
        })
        return;
     }
